...
`event_admin_list`. A working Keycloak event-store provider is required;
configuring a logging listener alone does not make past logs queryable here.
Retention and expiration determine how far back queries can reach. Enabling
storage does not reconstruct earlier activity.

Admin event representations additionally depend on Keycloak's Include
...
| `event_login_list` | `clientId` | Client identifier stored on the event, such as `my-app`; not the client's internal UUID |
| `event_login_list` | `userId` | Internal user ID, not username |
| `event_login_list` | `ipAddress` | Source IP value; not a CIDR range |
| `event_login_list` | `types` | Case-sensitive event names, e.g. `["LOGIN", "LOGIN_ERROR"]`; the tool can query other user-event types too |
| `event_admin_list` | `authClient` | Administrator's authenticating client identifier stored in `authDetails.clientId`; not the client being changed |
| `event_admin_list` | `authUser` | Administrator's internal user ID; not the affected user |
| `event_admin_list` | `operationTypes` | Case-sensitive operations, e.g. `["CREATE", "UPDATE", "DELETE", "ACTION"]` |
| `event_admin_list` | `resourceTypes` | Case-sensitive resource names, e.g. `["USER", "CLIENT", "REALM"]` |
| `event_admin_list` | `resourcePath` | Stored relative resource path, e.g. `users/<user-id>`; `users/*` matches paths below users |
...
environment, adding these non-secret settings to the MCP client's environment:
